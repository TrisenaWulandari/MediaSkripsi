...
function setCurTime1_1(X) {
  if (X == 1) {
    ...
    video1_1.currentTime = 53;
  } else if (X == 3) {
    video1_1.currentTime = 58;
  } else if (X == 4) {
    video1_1.currentTime = 110;
  } else if (X == 5) {
    video1_1.currentTime = 236;
  }
}
...
function setCurTime2_2(X) {
  if (X == 1) {
    ...
    video2_2.currentTime = 25;
  } else if (X == 3) {
    video2_2.currentTime = 56;
  }
}
...
$("#selesai").on("click", function (event) {
  ...
  set(ref(db, "selesai" + "esaiA/" + nama), {
    nama: nama,
    kelas: kelas,
    latihanA: document.getElementById("latihan1").value,
  });
  Swal.fire({
    icon: "success",
...
function AddItemToTableP22(nama, p2, j2, p3, j3, p4, j4, p5, j5) {
  ...
  latihanA.appendChild(tr);
}

function AddAllItemsToTableP22(latihan1) {
  latihanA.innerHTML = "";
  latihan1.forEach((element) => {
...
function GetAllDataRealtimeP22() {
  ...
  onValue(dbRef, (snapshot) => {
    ...
    snapshot.forEach((childSnapshot) => {
      latihan1.push(childSnapshot.val());
    });
